...
public class EpassportTest extends BaseClass{
	...
	@Test
	public void verifyEpassportForm() throws InterruptedException, IOException, TesseractException {
	    ...
	    try {
			waitEpassport.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("(//a[text()='Apply for ePassport'])[1]"))).click();
		...
		for (String windowId : driver.getWindowHandles()) {
			...
			try {
				waitApplyForPassport.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(text(),'Apply for Passport')]"))).click();
			...
			//	e.printStackTrace();
				...
				waitDropdown.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("(//div[@class='mat-select-arrow-wrapper'])[1]")));
				...
				waitNearestEnrollmentCenterText.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h3[contains(.,'Please choose the nearest enrollment center')]")));
				waitDropdownAppointment= new WebDriverWait(driver, Duration.ofSeconds(10));
				waitDropdownAppointment.until(ExpectedConditions.elementToBeClickable(By.xpath("(//div[@class='mat-select-arrow-wrapper'])[2]")));
				...
				while(!optionFourValue.equals(" NE, Paris ")) {
					...
					System.out.println("Conditon needed to check to verify correct selection! and to click retry for option 4 value");
					break;
				...
				waitListOfcurrentDates.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//tbody[contains (@class, 'ng-tns')]")));
				...
				waitSelectAnHourElement.until(ExpectedConditions.elementToBeClickable(By.cssSelector("div[class*='mat-chip-list-wrapper']")));
		        ...
		        waitCaptchaElement = new WebDriverWait(driver, Duration.ofSeconds(10));
		        waitCaptchaElement.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//img[@class='captcha-img']")));
		        ...
		        Mat captchaImage = Imgcodecs.imread(captchaImagePath);

		        // Convert the captcha image to grayscale
		        Mat grayImage = new Mat();
		        Imgproc.cvtColor(captchaImage, grayImage, Imgproc.COLOR_BGR2GRAY);

		        // Apply thresholding to binarize the captcha image
		        Mat binaryImage = new Mat();
		        Imgproc.threshold(grayImage, binaryImage, 0, 255, Imgproc.THRESH_BINARY_INV | Imgproc.THRESH_OTSU);

		        // Save the pre-processed captcha image
		        String preprocessedCaptchaImagePath = "D:\\cabtmDashboardAutomation\\testSelenium\\tessdata\\preprocessed_captcha.png";
		        Imgcodecs.imwrite(preprocessedCaptchaImagePath, binaryImage);

		        // Perform OCR on the pre-processed captcha image
		        ITesseract image = new Tesseract();
		        String datapath = "D:\\cabtmDashboardAutomation\\testSelenium\\tessdata\\";
		        image.setDatapath(datapath);
		        image.setTessVariable("user_defined_dpi", "300");
		        String captchaText = image.doOCR(new File(preprocessedCaptchaImagePath));
